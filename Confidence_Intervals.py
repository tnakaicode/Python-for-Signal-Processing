...
print('--Interval trapped correct value ', np.logical_and(phat - epsilon_n <=
                                                          0.5, 0.5 <= (epsilon_n + phat)).mean() * 100, '% of the time')
...
bs = [np.random.choice(xs[:, 0], size=len(
    xs[:, 0])).mean() for i in range(100)]
...
delta = fsolve(lambda delta: 0.95 -
               kbs.integrate_box(phat[0] - delta, phat[0] + delta), 0.1)[0]
...
ax.vlines(phat[0] - delta, 0, 12, lw=4., linestyle='--', color='gray')
ii = i[np.where(np.logical_and(i < phat[0] + delta, i > phat[0] - delta))]
...
def compute_bootstrap_CI(x, nboot=100):
    phat = x.mean()
    bs = [np.random.choice(
        x, size=len(xs)).mean() for i in range(nboot)]
    kbs = gaussian_kde(bs)  # kernel density estimate
    delta = fsolve(lambda delta: 0.95 -
                   kbs.integrate_box(phat - delta, phat + delta), 0.1)[0]
...
upper_b, lower_b = zip(*[compute_bootstrap_CI(xs[:, i])
...
patients = ['M'] * 10 + ['F'] * 14
# use the first 13 slots for survivors
sample = np.random.permutation(patients)[:13]
print(sample)
...
# count males in first group of 10 males total
