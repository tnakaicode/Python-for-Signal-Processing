...
fig, ax = plt.subplots()
v = np.arange(1, 7) + np.float64arange(1, 7)[:, None]
# some tweaks to get a float out


def foo(i): return density(z)[Integer(i)].evalf()


Zmass = np.array(map(foo, v.flat), dtype=np.float32).reshape(6, 6)

ax.pcolor(np.arange(1, 8), np.arange(1, 8), Zmass, cmap="jet")
...
def samples_z7(): return stats.sample(x, sympy.Eq(z, 7))  # Eq constrains Z
...
# generate samples conditioned on z=7
def samples_z7(): return stats.sample(x, Eq(z, 7))  # Eq constrains Z
