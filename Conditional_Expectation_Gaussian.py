...
from sympy import stats, sqrt, oo, use, det
...
R = Matrix([[sigma_x**2, sigma_xy],
            [sigma_xy, sigma_y**2]])
fxy = 1 / (2 * pi) / np.sqrt(det(R)) * np.exp((-Matrix([[x, y]]) * R.inv() * Matrix([[x], [y]]))[0, 0] / 2)
...
fcond2 = fcond.subs(sigma_x**2 * sigma_y**2 - sigma_xy**2,
                    u)  # substitute as hint to integrate
