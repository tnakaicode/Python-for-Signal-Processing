...
def db20(W,Nfft=None):
    ...
    if Nfft is None: # assume W is DFT 
        ...
        return 20*log10(abs( fft.fft(W,Nfft)/np.sqrt(Nfft) ) )
...
x=np.array(dftmatrix(64,64)[:,5].real).flatten() # convert to numpy np.array
n = arange(len(x))
