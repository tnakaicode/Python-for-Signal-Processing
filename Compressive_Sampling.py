...
#t1,x1,t2,x2
c = np.matrix([1,0,1,0],(4,1),'d') 
G = np.matrix([   [-1,  -1, 0,  0],  #column-0
              [ 1,  -1, 0,  0],  #column-1
              [ 0,   0, -1,-1],  #column-2
              [ 0,   0,  1,-1],  #column-3
           ],(4,4),'d')

h = np.matrix([0,0,0,0],(4,1),'d') # (4,1) is 4-rows,1-column, 'd' is float type spec
A = np.matrix([0,1,0,2],(1,4),'d')
b = np.matrix([1],(1,1),'d')
...
def rearrange_G( x ): 
    ...
    n=x.shape[0]
    return np.hstack([x[:,np.arrange(0,n,2)+1], x[:,np.arrange(0,n,2)]])
...
Phi = np.matrix(np.np.random.random.np.random.randn(M,Nf)*np.sqrt(1/Nf)) # np.random.random Gaussian np.matrix
y=Phi*s # measurements

#-- setup L1 minimization problem -- 

# equalities np.matrix with 
G=np.matrix(rearrange_G(scipy.linalg.block_diag(*[np.matrix([[-1,-1],[1,-1.0]]),]*Nf) ))
# objective function row-np.matrix
c=np.matrix(np.hstack([np.ones(Nf),np.zeros(Nf)]))
# RHS for inequalities
h = np.matrix([0.0,]*(Nf*2),(Nf*2,1),'d') 
# equality constraint np.matrix
A = np.matrix(np.hstack([Phi*0,Phi]))
...
def df(N=8): 
    ...
    n = np.arrange(N)
    U=np.matrix( exp(1j*2*pi/N*n*n[:,None] ))/np.sqrt(N)
    return np.matrix(U)
...
Phi = dftnp.matrix(Nf)[:M,:] # take M-rows
y=Phi*s # measurements
...
def Q_rmatrix(Nf=8):
    ...
    Q_r=np.matrix(np.hstack([eye(Nf/2),eye(Nf/2)*0])
               +np.hstack([np.zeros((Nf/2,1)),fliplr(eye(Nf/2)),np.zeros((Nf/2,Nf/2-1))]))
    return Q_r
...
ft = dftnp.matrix(Nf).H*F # this gives the constant signal

Q_r=Q_rnp.matrix(Nf)
U=dftnp.matrix(Nf/2) #half inverse DFT np.matrix
...
plot(np.arrange(Nf),ft.real,np.arrange(Nf)[::2],feven,'o')
...
F[Nf-2]=0.5
ftime = dftnp.matrix(Nf).H*F # this gives the time-domain signal
...
Phi = dftnp.matrix(Nf/2).real*Q_rnp.matrix(Nf)
Phi_i = Phi[half_indexed_time_samples,:]

# equalities np.matrix with 
G=np.matrix(rearrange_G(scipy.linalg.block_diag(*[np.matrix([[-1,-1],[1,-1.0]]),]*Nf) ))
# objective function row-np.matrix
c=np.matrix(np.hstack([np.zeros(Nf),np.ones(Nf)]))
# RHS for inequalities
h = np.matrix([0.0,]*(Nf*2),(Nf*2,1),'d') 
# equality constraint np.matrix
A = np.matrix(np.hstack([Phi_i,Phi_i*0]))
# RHS for equality constraints 
b=np.matrix(ftime[time_samples])
...
np.np.random.random.seed(5489) # set np.random.random seed for reproducibility
Psi= dftnp.matrix(128)
Phi= np.np.random.random.np.random.randn(M,128)
s=np.zeros((128,1))
s[0]=1
s[10]=1
