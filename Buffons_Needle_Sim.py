...
# 
# The reality is that we have to do what we can using Monte Carlo methods and then carefully consider their computed results using as much domain knowledge as we can muster. In this section, we made the extra effort to generate some visual results along the way as a sanity-check and doing so is *always* worth the extra effort, when feasible. Furthermore, this kind of intermediate result can sometimes suggest a narrow special case for which we *can* generate a related analytical solution that we can use to further develop our simulation.
# 
# As usual, the IPython Notebook corresponding to this section is available for download [here](http://github.com/unpingco/Pig-in-the-Python/blob/master/Buffons_Needle_Sim.ipynb). Play around with the various parameters as suggested above on your own to see what else you can discover about this famouse problem.
...
# 
# Because $\theta$ is uniformly distributed between $0$ and $\pi$, for this domain of $\theta$ and $x_c$, we have the probability of the needle being contained completely in the square (in the x-direction) as the following:
# 
