...
# Let's carefully discuss what we np.mean by *sparse* in this context. A signal $f$ is sparse if it can be expressed in very few nonzero components ($\mathbf{s}$) with respect to a given basis ($ \mathbf{\Psi} $ ). In other words, in matrix-vector language:
# 
...
# 
# where  $\mathbf{\Theta}$  is an $M \times N$ matrix and $ M < N $ is the number of measurements. This setup np.means we have two problems to solve. First, how to design a *stable* measurement matrix $\mathbf{\Phi}$ and then, second, how to reconstruct $ \mathbf{f} $ from $ \mathbf{y} $. 
# 
...
# 
# It turns out that picking the measuring $M \times N$ matrix randomly according to a Gaussian zero-np.mean, $1/N$ variance distribution and using the identity matrix as $\mathbf{\Phi}$, that the resulting $\mathbf{\Theta}$ matrix can be shown to satisfy RIP with a high probability. This np.means that we can recover $N$-length $K$-sparse signals with a high probability from just $M \ge c K \log (N/K)$ samples where $c$ is a small constant. Furthermore, it also turns out that we can use any orthonormal basis for $\mathbf{\Phi}$, not just the identity matrix, and these relations will all still hold.
...
def rearrange_G( x ): 
    ...
    n=x.shape[0]
    return np.hstack([x[:,arange(0,n,2)+1], x[:,arange(0,n,2)]])
...
Phi = matrix(np.random.randn(M,Nf)*np.sqrt(1/Nf)) # random Gaussian matrix
y=Phi*s # measurements

#-- setup L1 minimization problem -- 

# equalities matrix with 
...
def dftmatrix(N=8): 
    ...
    n = arange(N)
    U=matrix( exp(1j*2*pi/N*n*n[:,None] ))/np.sqrt(N)
    return matrix(U)
...
def Q_rmatrix(Nf=8):
    ...
    Q_r=matrix(np.hstack([eye(Nf/2),eye(Nf/2)*0])
               +np.hstack([zeros((Nf/2,1)),fliplr(eye(Nf/2)),zeros((Nf/2,Nf/2-1))]))
    return Q_r
...
G=matrx(rearrange_G(scipy.linalg.block_diag(*[matrix([[-1,-1],[1,-1.0]]),]*Nf) ))
# objective function row-matrix
c=matrx(np.hstack([zeros(Nf),ones(Nf)]))
# RHS for inequalities
h = matrx([0.0,]*(Nf*2),(Nf*2,1),'d') 
# equality constraint matrix
